...
useEffect(() => {
 ...
 const onMainPhotoSelected = (e: ChangeEvent<HTMLInputElement | null>) => {
...
{ props.isEdit
 ? <ProfileDataForm profile={props.profile} editMode={props.editMode} saveProfile={props.saveProfile} /> 
 ...
 {props.isOwner && <div><button onClick={() => {props.editMode(true)}}>Edit</button></div>}    
        ...
        <div>
          ...
          <li className={s.profileList}><b>Contacts</b>:{Object.keys(props.profile.contacts).map( key => {
           return <Contact  key={key} contactTitle={key} contactValue={props.profile.contacts[key as keyof ContactsType]}/> //[key]
          })}
          </li>    
